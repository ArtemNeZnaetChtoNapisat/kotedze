...
import { Icon_left_page, Icon_right_page, Svg_account_nick, Svg_verefy, Icon_burger_menu, Svg_logo, Icon_home, Icon_add, Icon_information,Icon_shop } from "./icon";
import Image from "next/image";
import style from "./styles/ui.module.css";
import { useState } from "react";
import setpromo from "../styles/setpromocode.module.css"
...
<div className="block_page"> 
    <button className={props.getPage >1 ? "button_page" : "buttonEnd"} title="Начальная страница">{props.getPage >1 ? <Link href="?page=1" title="Начальная страница">1...</Link>: "1..."}</button>
    <button className={props.getPage >1 ? "button_page" : "buttonEnd"} title="Предыдущая страница">{props.getPage >1 ? <Link href={left} title="Предыдущая страница"><Icon_left_page/></Link> : <Icon_left_page/>}</button>
    <button>{props.getPage}</button>
    <button className={props.getPage <props.maxPage ? "button_page" : "buttonEnd"} title="Следующая страница"> {props.getPage <props.maxPage ? <Link href={right} title="Следующая страница"><Icon_right_page/></Link> : <Icon_right_page/>}</button>
    
    <button className={props.getPage <props.maxPage ? "button_page" : "buttonEnd"} title="Последняя страница">{props.getPage <props.maxPage ? <Link href={max} title="Последняя страница">...{props.maxPage}</Link> : "..."+props.maxPage}</button>
</div>
    );
}
...
function Content(props) {
    ...
    let user_id = "/user/"+props.user_id;
        ...
        return (
              <>
              <div className={style.cartochka_content}>
            
                <div className={style.information__content}>
                <div>
                    <div className={style.image_content_block}>
                    {/* <img className="image_content"  src={props.img} alt="image_promo" /> */}
                    <Image loading = "lazy" placeholder="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAAAXNSR0IArs4c6QAAAA1JREFUGFdj2L9//38AB7sDPcyW+IQAAAAASUVORK5CYII="  src={props.img} alt="image_promo" width={192} height={192} style={{width: '100%',height: 'auto',}} className="image_dec"/>
                    <Image src={props.img}  placeholder="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAAAXNSR0IArs4c6QAAAA1JREFUGFdj2L9//38AB7sDPcyW+IQAAAAASUVORK5CYII=" alt="image_promo" width={92} height={92} style={{width: '100%',height: 'auto',}} className="image_mob" loading = 'lazy'/>
                    {/* <Image src={props.img} alt="image_promo" fill={false} width={92} height={92}  /> */}
                    </div>
                </div>
                <div>
                    <div id={style.text_content}> <Link href={id}><h2>{props.title}</h2></Link></div>
                    <div id={style.description_content}><p>{props.description}</p></div>
                    <div className={style.cena_cartochka_content}><p>{props.skidka==0 ? props.cena+"₽" : props.skidka+"%"}</p> <del>{(Number(props.type ) && Number(props.decena)!=0 ) ? props.decena+"₽" : null}</del></div>
                    </div>
                </div>
                <div className={style.buttom_content}>
                <Link href={user_id} > <div id={style.auchor_nickname_content}><Svg_account_nick /> {props.nickname} {props.verefy ? <Svg_verefy /> : null}</div> </Link>
                        <Link href={id} className={style.button_content}> Подробнее</Link>
                    </div>
            </div>
              </>
        );
...
function Top_block(props) {
    const [active, setActive] = useState(true);
    const [burger, setBurger] = useState(false);
      
        //  let path = getCookie("hash")!=undefined ? "/user" : "/auth";
        //  let auth = getCookie("hash")!=undefined ? true : false;
        console.log(burger)
        return (<>
        <div className={active ? "top_block" : "top_block_close"}>
            <div className="button_top_block">
                <button className="burger_menu" onClick={()=>{!burger ? setBurger(true) : setBurger(false)}} title="burger"><Icon_burger_menu/></button> 
                <a href='../' title="Kotedze logo"><Svg_logo /></a> {/*<Link to={path} id="account_svg"><Svg_account /></Link>*/}
                </div>
        </div>
         <Burger_menu state={props.state} active={burger} burger={setBurger}/>
        </>
        );
      };
      ...
      function Burger_menu(props){
        ...
        let path = true ? "/setpromocode" : "/auth";
        return (<>
        <div className={props.active ? "fon_burger_menu_active" : "fon_burger_menu" } onClick={()=>props.burger(false)}></div>
        <ul className={props.active ? "list_burger_menu_active":"list_burger_menu"}>
         <li > <Link href="/" className={state == "home" ?"burger_link_active": "burger_link"}><Icon_home/> Главная</Link></li>
          {/* <li>  <Link href={path} className={state == "setpromocode" ?"burger_link_active": "burger_link"}><Icon_add/> Добавить промокод</Link> </li> */}
          <li >  <Link href="/about" className={state == "about" ?"burger_link_active": "burger_link"}><Icon_information/> О нас</Link></li>
          {/* <li >  <Link to="/about" className={state == "about" ?"burger_link_active": "burger_link"}><Icon_information/>Партнерская програма</Link></li> */}
          <div className="contain_burger_menu"></div>
          <li >  <Link href="/shop/" className={state == "shop" ?"burger_link_active": "burger_link"}><Icon_shop/> Все магазины</Link></li>
         
         <Link href="/shop/litres"  className="link_shop_burger_menu">Литрес, </Link>
         <Link href="/shop/megamarket"  className="link_shop_burger_menu">Мегамаркет, </Link>
         <Link href="/shop/yandex-market"  className="link_shop_burger_menu">Яндекс Маркет, </Link>
         <Link href="/shop/aliexpress"   className="link_shop_burger_menu">Aliexpress, </Link>
         <Link href="/shop/ozon"  className="link_shop_burger_menu">Ozon, </Link>
         <Link href="/shop/m-video"  className="link_shop_burger_menu">М.Видео, </Link>
         <Link href="/shop/samokat"  className="link_shop_burger_menu">Самокат, </Link>
         <Link href="/shop/plus-yandex"  className="link_shop_burger_menu">Яндекс Плюс, </Link>
         <Link href="/shop/sber" className="link_shop_burger_menu">СберПрайм, </Link>
         <Link href="/shop/ivi"  className="link_shop_burger_menu">Иви</Link>
      ...
      function Input(props){
        return(<>
        <p className={setpromo.input_title}>{props.title}</p>
        <div className={setpromo.input_block}>
        <div className={setpromo.icon_input}>{props.icon}</div>
  <input className={setpromo.input_text} type="text" placeholder={props.placeholder} autocomplete="off" name={props.name}></input>
        </div>
  </>);
      }
      function Textarea(props){
        return(<>
        <p className={setpromo.input_title}>{props.title}</p>
  <textarea className={setpromo.input_textarea} type="text" placeholder={props.placeholder} autocomplete="off" name={props.name}/>
  </>);
      }
  
      function Button(props){
        return(<>
      <button className={setpromo.button_primary} onClick={props.onClick}>Добавить скидку</button>
  </>);
      }
  

export {Pagination, Content, No_content, Top_block, Input, Textarea, Button};
